...
const database = new AuditDatabase(config.TASK_DB_PATH);
...
if (!database.getManagedService('go-hello')) {
  const now = new Date().toISOString();
  database.saveManagedService({
    id: 'go-hello',
    name: 'Go Hello API',
    runtime: 'go',
    workingDir: 'managed-services/go-hello',
    env: { PORT: '8090', SERVICE_NAME: 'go-hello' },
    createdAt: now,
    updatedAt: now,
  });
}
if (!database.getManagedService('node-hello')) {
  const now = new Date().toISOString();
  database.saveManagedService({
    id: 'node-hello',
    name: 'Node Hello API',
    runtime: 'node',
    workingDir: 'managed-services/node-hello',
    env: { PORT: '8091', SERVICE_NAME: 'node-hello' },
    createdAt: now,
    updatedAt: now,
  });
}
if (!database.getManagedService('vue-hello')) {
  const now = new Date().toISOString();
  database.saveManagedService({
    id: 'vue-hello',
    name: 'Vue Hello App',
    runtime: 'vue',
    workingDir: 'managed-services/vue-hello',
    env: { PORT: '8092' },
    createdAt: now,
    updatedAt: now,
  });
...
async function tracked<T>(request: any, serviceId: string, actionId: string, params: unknown, fn: () => Promise<T>) {
  const task = tasks.create(serviceId, actionId, actorFor(request), request.id, params);
  const started = Date.now();
  database.updateTask(task.id, { status: 'running', startedAt: new Date().toISOString() });
  tasks.event(task.id, 'system', `Starting ${actionId}`);
  try {
    ...
    tasks.event(task.id, 'system', `Completed ${actionId} in ${Date.now() - started}ms`);
    return { task, result };
  } catch (error: any) {
    database.updateTask(task.id, {
      status: 'failed',
    ...
    tasks.event(task.id, 'stderr', error.message);
    throw error;
  }
}
...
for (const action of ['start', 'stop', 'restart'] as const) {
  app.post(`/api/services/:serviceId/${action}`, async (request) => {
    ...
    const task = tasks.create(service.id, `compose.${action}Service`, actorFor(request), request.id, {
      service: service.id,
    });
    tasks.run(task, 'docker', infra.composeArgs(action === 'start' ? 'up' : action, [service.compose!]));
...
app.post('/api/tasks', async (request) => {
  ...
  if (body.actionId === 'compose.restartUnhealthy') {
    ...
    const task = tasks.create('kafka', body.actionId, actorFor(request), request.id, body.params);
    tasks.run(task, 'docker', infra.composeArgs('restart', unhealthy));
    return { type: 'task', taskId: task.id, status: 'running', eventUrl: `/api/tasks/${task.id}/events` };
  }
  const task = tasks.create('mysql', body.actionId, actorFor(request), request.id, body.params);
  tasks.run(task, 'docker', infra.composeArgs(body.actionId === 'compose.startAll' ? 'up' : 'stop', names));
  return { type: 'task', taskId: task.id, status: 'running', eventUrl: `/api/tasks/${task.id}/events` };
});
...
app.get('/api/tasks/history', async (request) => {
  const query = z
    ...
    .parse(request.query);
  return { rows: database.listTasks(query.limit, query.offset, query), limit: query.limit, offset: query.offset };
});
app.delete('/api/tasks', async (request) => {
  const { taskIds } = z.object({ taskIds: z.array(z.string().min(1)).min(1).max(100) }).parse(request.body);
  const deleted = database.deleteTasks(taskIds);
  return { deleted, skipped: taskIds.length - deleted };
});
app.get('/api/tasks/:taskId', async (request) => {
  const task = database.getTask(z.object({ taskId: z.string() }).parse(request.params).taskId);
...
app.get('/api/tasks/:taskId/events', async (request, reply) => {
  ...
  const channel = sse(reply);
  database.getEvents(taskId, after).forEach((event) => channel.send('log', event, event.id));
...
app.get('/api/app-services', async () => ({ rows: managedServices.list() }));
app.post('/api/app-services', async (request) => {
  const body = managedServiceBody.parse(request.body);
  const now = new Date().toISOString();
  const service = database.saveManagedService({
...
app.put('/api/app-services/:serviceId', async (request) => {
  ...
  const current = database.getManagedService(serviceId);
  if (!current) throw Object.assign(new Error('Managed service not found'), { statusCode: 404 });
  const body = managedServiceBody.parse(request.body);
  const service = database.saveManagedService({ ...current, ...body, updatedAt: new Date().toISOString() });
  return managedServices.status(service);
});
app.delete('/api/app-services/:serviceId', async (request) => {
  const { serviceId } = managedServiceId.parse(request.params);
  await managedServices.stop(serviceId);
  if (!database.deleteManagedService(serviceId))
...
for (const action of ['start', 'stop', 'restart'] as const) {
  app.post(`/api/app-services/:serviceId/${action}`, async (request) => {
    const { serviceId } = managedServiceId.parse(request.params);
    return managedServices[action](serviceId);
  });
}
app.get('/api/app-services/:serviceId/logs/events', async (request, reply) => {
  const { serviceId } = managedServiceId.parse(request.params);
  if (!managedServices.get(serviceId)) throw Object.assign(new Error('Managed service not found'), { statusCode: 404 });
  const { tail } = z.object({ tail: z.coerce.number().int().min(1).max(1000).default(100) }).parse(request.query);
  const channel = sse(reply);
  managedServices.eventsFor(serviceId, tail).forEach((event) => channel.send('log', event, event.id));
  const listener = (event: any) => channel.send('log', event, event.id);
...
app.post('/api/docker/tasks', async (request) => {
  ...
  const task = tasks.create('docker', `docker.${commandId}`, actorFor(request), request.id, { commandId });
  tasks.run(task, command.command, [...command.args]);
  return { type: 'task', taskId: task.id, status: 'running', eventUrl: `/api/tasks/${task.id}/events` };
});
...
app.post('/api/gcloud/tasks', async (request) => {
  ...
  const task = tasks.create('gcloud', 'gcloud.runApprovedAction', actorFor(request), request.id, body);
  const args = [...action.args, ...(body.project ? ['--project', body.project] : [])];
  tasks.run(task, 'docker', infra.gcloudArgs(args, `local-infra-task-${task.id}`));
  return { type: 'task', taskId: task.id, status: 'running', eventUrl: `/api/tasks/${task.id}/events` };
});
...
app.get('/api/spanner/saved-queries', async (request) => {
  const { instanceId, databaseId } = z
    ...
    .parse(request.query);
  return { rows: database.listSavedSpannerQueries(instanceId, databaseId) };
});
app.post('/api/spanner/saved-queries', async (request) => {
  const body = savedSpannerQuery.parse(request.body);
  const now = new Date().toISOString();
  return database.saveSpannerQuery({ id: `spanner-query-${randomUUID()}`, ...body, createdAt: now, updatedAt: now });
});
app.delete('/api/spanner/saved-queries/:queryId', async (request) => {
  ...
  if (!database.deleteSavedSpannerQuery(queryId))
