...
export class ManagedServiceRunner {
  ...
  async list() {
    return (await this.database.listManagedServices()).map((service) => this.status(service));
  }

  async get(id: string) {
    const service = await this.database.getManagedService(id);
  ...
  private cwdFor(service: ManagedService) {
    ...
    if (!existsSync(cwd) || !statSync(cwd).isDirectory())
      throw Object.assign(new Error(`Working directory does not exist: ${service.workingDir}`), { statusCode: 400 });
    return cwd;
  }

  private async event(serviceId: string, stream: ManagedServiceEvent['stream'], text: string) {
    const event = await this.database.addManagedServiceEvent({
      serviceId,
      stream,
  ...
  private capture(serviceId: string, stream: 'stdout' | 'stderr', chunk: Buffer) {
    for (const line of chunk.toString().split(/\r?\n/).filter(Boolean)) void this.event(serviceId, stream, line);
  }
  ...
  async start(id: string) {
    ...
    const environment = { ...process.env, ...service.env };
    if (service.runtime === 'go') {
      const goCacheRoot = '/tmp/local-infra-control-center-go';
      mkdirSync(`${goCacheRoot}/build`, { recursive: true });
      mkdirSync(`${goCacheRoot}/mod`, { recursive: true });
      environment.GOCACHE = `${goCacheRoot}/build`;
      environment.GOMODCACHE = `${goCacheRoot}/mod`;
    }
    ...
    await this.event(id, 'system', `Starting ${command.label} in ${service.workingDir}`);
    ...
    child.on('error', (error) => void this.event(id, 'stderr', error.message));
    child.on('close', (code, signal) => {
      this.running.delete(id);
      void this.event(id, 'system', `Service stopped; exit=${code ?? signal ?? -1}`);
  ...
  async stop(id: string) {
    ...
    await this.event(id, 'system', 'Stop requested');
  ...
  async eventsFor(id: string, tail: number) {
